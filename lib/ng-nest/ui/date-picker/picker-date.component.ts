...
export class XPickerDateComponent extends XPickerDateProperty implements OnChanges {
  ...
  dateClick(date: Date | string) {
    this.model = date as Date;
    this.modelChange.emit(date as Date);
    this.cdr.markForCheck();
  }

  trackByTitleNode(_index: number, item: string) {
    return item;
  }

  trackByWeekNode(_index: number, item: Date) {
    return item;
  }
}
