...
export class XBackTopProperty extends XPropertyFunction(X_BACK_TOP_CONFIG_NAME) {
   ...
   */
  readonly right = input<string, XNumber>(this.config?.right ?? '2.5rem', { transform: XToCssPixelValue });
  /**
   * @zh_CN 距离底部的距离
   * @en_US Distance from bottom
   */
  readonly bottom = input<string, XNumber>(this.config?.bottom ?? '2.5rem', { transform: XToCssPixelValue });
  /**
   * @zh_CN 滚动多少高度后显示
   * @en_US Display after scrolling how much height
   */
  readonly visibilityHeight = input<number, XNumber>(this.config?.visibilityHeight ?? 200, { transform: XToNumber });
  /**
   * @zh_CN 内容模板
